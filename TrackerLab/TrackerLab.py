...
class Window(QMainWindow):
    ...
    def openFilesDialog(self):
        ...
        self.files, _ =  QtWidgets.QFileDialog.getOpenFileNames(self, 'Open Files...', self.dir, 'TDMS Files (*.tdms);;TIFF Files (*.tif)') # 'All Files (*)'  # for MacOs the native filedialog finder crashes sometimes, you might use options=QtWidgets.QFileDialog.DontUseNativeDialog
        if self.files:
            self.imageFiles = []
            ...
            for file in self.files:
                if fnmatch.fnmatch(file,'*_video.tdms') or fnmatch.fnmatch(file,'*.tif'):
